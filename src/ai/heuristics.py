...
def eval_pression(state, player_id, mobility=0, opp_mobility=0):
    """
    Niveau Difficile.

    Cette heuristique évalue une position en combinant :
    - le score officiel actuel,
    - le potentiel de capture des deux joueurs.
    """
    opponent_id = 1 - player_id
    ...
    moves = state.get_legal_moves(player_id)
    ...
    for move in moves:
        if len(move) == 4:
            ...
            if state.board[nr][nc] != EMPTY:
                attack_score += PIECE_VALUES[state.board[nr][nc]] * 10

    opp_attack_score = 0

    opp_moves = state.get_legal_moves(opponent_id)
    ...
    for move in opp_moves:
        if len(move) == 4:
            ...
            if state.board[nr][nc] != EMPTY:
                opp_attack_score += PIECE_VALUES[state.board[nr][nc]] * 10
    ...
    # La note finale
    stall_penalty = state.moves_without_capture * 5
    return score_diff + pressure_diff - stall_penalty
